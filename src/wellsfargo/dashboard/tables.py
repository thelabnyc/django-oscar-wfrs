...
class CreditApplicationTable(DashboardTable):
    ...
    merchant_name = Column(verbose_name=_("Merchant Name"), orderable=False)
    application_source = Column(verbose_name=_("Application Source"), orderable=False)
    user = TemplateColumn(
    ...
    submitting_user = TemplateColumn(
        ...
        order_by=(
            ...
            "submitting_user__username",
        ),
    )
    status = Column(verbose_name=_("Application Status"), orderable=False)
    ...
    created_datetime = TZAwareDateTimeColumn(
        verbose_name=_("Created On"), order_by="created_datetime", format="D, N j Y, P"
    )
...
class TransferMetadataTable(DashboardTable):
    merchant_reference = LinkColumn(
        "wfrs-transfer-detail", args=[A("merchant_reference")]
    )
    masked_account_number = Column(
        verbose_name=_("Account Number"), order_by=("last4_account_number")
    )
    ...
    type_name = Column(verbose_name=_("Type"), order_by=("type_code",))
    ticket_number = Column(verbose_name=_("Ticket Number"))
    financing_plan_number = Column(
        verbose_name=_("Plan Number"), order_by=("financing_plan__plan_number",)
    )
...
class PreQualificationTable(DashboardTable):
    uuid = LinkColumn(
        "wfrs-prequal-detail", args=[A("uuid")], verbose_name=_("UUID"), orderable=False
    )
    merchant_name = Column(
        verbose_name=_("Merchant Name"), accessor=A("merchant_name"), orderable=False
    )
    ...
    status_name = Column(verbose_name=_("Status"), orderable=False)
    credit_limit = TemplateColumn(
        verbose_name=_("Pre-Qual Credit Limit"),
        template_name="wfrs/dashboard/_prequal_row_credit_limit.html",
        orderable=False,
    )
    customer_response = Column(verbose_name=_("Customer Response"), orderable=False)
    sdk_application_result = Column(
        verbose_name=_("SDK Application Result"), orderable=False
    )
    merchant_num = Column(verbose_name=_("Merchant Number"), orderable=False)
    customer_initiated = Column(verbose_name=_("Customer Initiated"), orderable=False)
    order_total = TemplateColumn(
        verbose_name=_("Order Total"),
    ...
    order_merchant_name = Column(verbose_name=_("Order Merchant Name"), orderable=False)
    created_datetime = TZAwareDateTimeColumn(
        verbose_name=_("Created On"), order_by="created_datetime", format="D, N j Y, P"
    )
