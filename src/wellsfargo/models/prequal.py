...
class PreQualificationRequest(models.Model):
    ...
    customer_initiated = models.BooleanField(
        _("Check was deliberately initiated by customer action"), default=False
    )
    email = models.EmailField(_("Email"), null=True, blank=True)
    first_name = models.CharField(_("First Name"), max_length=15)
    middle_initial = models.CharField(
        _("Middle Initial"), null=True, blank=True, max_length=1
    )
    ...
    def status_name(self):
        ...
        return get_prequal_trans_status_name(
            PREQUAL_TRANS_STATUS_REJECTED, self.customer_initiated
        )
    ...
    def resulting_order(self):
        ...
        order = (
            Order.objects.filter(email_matches & date_matches)
...
class PreQualificationResponse(models.Model):
    request = models.OneToOneField(
        PreQualificationRequest, related_name="response", on_delete=models.CASCADE
    )
    status = models.CharField(
        _("Transaction Status"), max_length=10, choices=PREQUAL_TRANS_STATUS_CHOICES
    )
    message = models.TextField(_("Message"))
    offer_indicator = models.CharField(_("Offer Indicator"), max_length=20)
    credit_limit = models.DecimalField(
        _("Credit Limit"), decimal_places=2, max_digits=12
    )
    ...
    def status_name(self):
        return get_prequal_trans_status_name(
            self.status, self.request.customer_initiated
        )

    @property
    def customer_response_name(self):
        return dict(PREQUAL_CUSTOMER_RESP_CHOICES).get(
            self.customer_response, self.customer_response
        )
    ...
    def full_application_url(self):
        ...
        return "{base_url}&mn={merchant_num}".format(
            base_url=self.application_url, merchant_num=merchant_num
        )
...
class PreQualificationSDKApplicationResult(models.Model):
    prequal_response = models.OneToOneField(
