...
class TransactionsAPIClient(WFRSGatewayAPIClient):
    ...
    def submit_transaction(self, trans_request, transaction_uuid=None, persist=True):
        ...
        resp = self.api_post(api_path,
            client_request_id=transaction_uuid,
            json=trans_request_data)
        ...
        transfer.ticket_number = resp_data.get('ticket_number', trans_request.ticket_number)
        transfer.financing_plan = plan
        transfer.auth_number = resp_data.get('authorization_number', trans_request.auth_number)
    ...
    def get_api_path(self, trans_request):
        ...
        api_path = '/credit-cards/private-label/new-accounts/v2/payment/transactions/{action}'.format(action=action)
        return api_path
